...
%code requires{
    ...
    struct YYSTYPE{
        ...
        std::vector<std::shared_ptr<Term>>  temp_regs;
...
%code{
    ...
    std::vector<std::shared_ptr<Term>> merge_registers(const std::vector<std::shared_ptr<Term>>& regs1, const std::vector<std::shared_ptr<Term>>& regs2, std::vector<std::shared_ptr<Term>>& perm_regs){
        std::vector<std::shared_ptr<Term>> regs;
        for(std::shared_ptr<Term> reg : regs1){
            if(find_in_vector(regs2, reg) == -1){
                regs.push_back(reg);
            }
        }
        for(std::shared_ptr<Term> reg : regs2){
            if(find_in_vector(perm_regs, reg) == -1){
                if(find_in_vector(regs, reg) == -1){
                    regs.push_back(reg);
                } else{
                    perm_regs.push_back(reg);
                }
            }
        }
        return regs;
...
predicate:        term IMPLICATION terms DOT    {
                ...
                | term DOT                      {
                                                    $$.lineno = $1.lineno;
                                                    $$.temp_regs = merge_registers($1.ts, $1.temp_regs);
                                                    std::vector<std::shared_ptr<Term>> arg_regs = $1.ts[0]->subterms;
                                                    if(query){
                                                    $$.str = compile_query($$.temp_regs);
                                                    } else{
                                                    $$.str = compile_program($$.temp_regs);
                                                    }
                                                }
;
term:         STRUCT LPAR terms RPAR    {
                                            ...
                                            $$.temp_regs = merge_registers($$.temp_regs, $3.ts);
                                            $$.temp_regs = merge_registers($$.temp_regs, $3.temp_regs);
                                        }
            | STRUCT                    {
                                            $$.lineno = $1.lineno;
...
terms:        terms COMMA term          {
                                            ...
                                            $$.temp_regs = merge_registers($1.temp_regs, $3.temp_regs, $$.perm_regs);
                                        }
            | term                      {
                                            $$.lineno = $1.lineno;
                                            $$.ts = $1.ts;
                                            $$.temp_regs = $1.temp_regs;
                                        }
;
%%
